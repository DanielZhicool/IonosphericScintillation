...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **PCHIP 3x Upsampling** | $288.9\ \mu\text{s}$ | $987.2\ \mu\text{s}$ | $334.5\ \mu\text{s}$ | $64.4\ \mu\text{s}$ | $2,989.8\text{ ops/s}$ | Monotonic cubic hermite interpolation ($2,000 \to 5,998$ pts; $3\times$ sampling rate) |
| **Hampel & Savitzky-Golay** | $424.5\ \mu\text{s}$ | $1.17\ \text{ms}$ | $478.8\ \mu\text{s}$ | $64.8\ \mu\text{s}$ | $2,088.5\text{ ops/s}$ | Fast $W=15$ outlier detection (`scipy.ndimage.median_filter`) + degree-2 polynomial smoothing |
| **Full Spectral Pipeline** | $48.87\ \text{ms}$ | $51.65\ \text{ms}$ | $50.01\ \text{ms}$ | $859.5\ \mu\text{s}$ | $20.0\text{ ops/s}$ | 4-channel Multitaper PSD ($K=7$), F-Test, Cross-Spectrum, Theoretical Phase Variance & IDVE |
| **CWT Spectrogram ($N=500$)** | $141.62\ \text{ms}$ | $171.68\ \text{ms}$ | $152.80\ \text{ms}$ | $11.54\ \text{ms}$ | $6.5\text{ ops/s}$ | Continuous Generalized Morse Wavelet transform |
| **SST Spectrogram ($N=500$)** | $149.61\ \text{ms}$ | $161.42\ \text{ms}$ | $155.92\ \text{ms}$ | $5.27\ \text{ms}$ | $6.4\text{ ops/s}$ | Synchrosqueezed Generalized Morse Wavelet transform |
...
| :---: | :---: | :--- | :---: | :---: | :---: | :---: | :---: |
| **$500$** | $\sim 8.3\text{ min}$ | Wavelet Sub-Window | $0.50\ \text{ms}$ | $0.28\ \text{ms}$ | $79.05\ \text{ms}$ | $141.78\ \text{ms}$ | $159.80\ \text{ms}$ |
| **$1,000$** | $\sim 16.7\text{ min}$ | Half-Transit Epoch | $0.66\ \text{ms}$ | $0.37\ \text{ms}$ | $86.98\ \text{ms}$ | $131.02\ \text{ms}$ | $171.14\ \text{ms}$ |
| **$2,000$** | $\sim 33.3\text{ min}$ | Single Target Transit | $0.61\ \text{ms}$ | $0.37\ \text{ms}$ | $104.73\ \text{ms}$ | $137.32\ \text{ms}$ | $188.06\ \text{ms}$ |
| **$5,000$** | $\sim 1.39\text{ hrs}$ | Extended Observation | $0.78\ \text{ms}$ | $0.55\ \text{ms}$ | $158.04\ \text{ms}$ | $153.05\ \text{ms}$ | $229.29\ \text{ms}$ |
| **$10,000$** | $\sim 2.78\text{ hrs}$ | Multi-Source Run | $1.10\ \text{ms}$ | $0.87\ \text{ms}$ | $247.95\ \text{ms}$ | $180.60\ \text{ms}$ | $316.78\ \text{ms}$ |
| **$50,000$** | $\sim 13.89\text{ hrs}$ | Overnight Epoch | $3.84\ \text{ms}$ | $5.00\ \text{ms}$ | $1.05\ \text{s}$ | $1.11\ \text{s}$ | $2.44\ \text{s}$ |
| **$86,400$** | **1.00 Day (24 hrs)** | **Full 24-Hour Day** | $7.15\ \text{ms}$ | $10.22\ \text{ms}$ | $1.70\ \text{s}$ | $1.59\ \text{s}$ | $3.78\ \text{s}$ |
| **$184,280$** | **2.13 Days (51.2 hrs)** | Archive Length (e.g. `23022013.PM6`) | $15.75\ \text{ms}$ | $23.67\ \text{ms}$ | $3.95\ \text{s}$ | $3.43\ \text{s}$ | $8.00\ \text{s}$ |
| **$260,793$** | **3.02 Days (72.4 hrs)** | Archive Length (e.g. `18012013me.PM6`) | $25.52\ \text{ms}$ | $42.99\ \text{ms}$ | $8.38\ \text{s}$ | $4.35\ \text{s}$ | $10.77\ \text{s}$ |
| **$432,000$** | **5.00 Days (120 hrs)** | **Full 5-Day Run** | $36.82\ \text{ms}$ | $65.55\ \text{ms}$ | $8.31\ \text{s}$ | $7.64\ \text{s}$ | $17.79\ \text{s}$ |
| **$506,069$** | **5.86 Days (140.6 hrs)** | Archive Length (e.g. `04012013me.PM6`) | $41.05\ \text{ms}$ | $66.53\ \text{ms}$ | $17.28\ \text{s}$ | $8.59\ \text{s}$ | $20.81\ \text{s}$ |

---

### 4.1. Algorithmic Complexity Insights
- **Filtering** $\mathcal{O}(N)$: The rolling Hampel median filter and Savitzky-Golay convolution exhibit linear scaling, processing an entire 5.86-day equivalent signal ($N = 506,069$) in just **41 ms** (and 24 hours in **7.15 ms**).
- **PCHIP Interpolation** $\mathcal{O}(N)$: Monotonic piecewise cubic spline evaluation scales linearly, taking under **67 ms** to upsample 5.86 days of continuous data.
...
To run the rapid unit test suite without benchmarks:
