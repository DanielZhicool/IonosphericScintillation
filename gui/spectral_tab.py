...
class SpectralTab(QWidget):
    ...
    def __init__(self, source_name, band_results, parent=None):
        ...
        self.band_tabs.addTab(
            self._create_band_widget(band_results.get("small"), SPECTRAL_BAND_SMALL),
            SPECTRAL_BAND_SMALL,
        )
        self.band_tabs.addTab(
            self._create_band_widget(band_results.get("large"), SPECTRAL_BAND_LARGE),
            SPECTRAL_BAND_LARGE,
        )
    ...
    def _build_psd_grid(self, graph, data_dict, periods, mask, p_min, p_max):
        ...
        for ch_name, row, col in channels:
            if ch_name not in data_dict:
                continue
            label = ch_name
            p = graph.ci.addPlot(row=row, col=col, title=label)
            ...
            if len(peaks) > 0:
                ...
                title_html = (
                    f"<span style='font-size:12pt;'>{label}</span><br>"
    ...
    def _build_ftest_grid(self, graph, data_dict, periods, mask, freqs_full, p_min, p_max):
        ...
        for ch_name, row, col in channels:
            ...
            if T0 is not None:
                ...
                title_html = (
                    f"<span style='font-size:12pt;'>{label}</span><br>"
                    f"<span style='color:#42A5F5; font-size:11pt; font-weight:bold;'>T₀ = {T0:.1f} s"
                )
                if p_min <= t2 <= p_max:
            ...
            p = graph.ci.addPlot(row=row, col=col, title=title_html)
            p.setLabel("bottom", "Period (Sec)")
            ...
            thresh_line = pg.InfiniteLine(
                pos=threshold,
                angle=0,
                pen=pg.mkPen("#FF4444", width=2.5, style=Qt.PenStyle.DashLine),
            )
    ...
    def _build_cross_spectrum(self, graph, cross_data, periods, mask, p_min, p_max):
        ...
        p_ima = graph.ci.addPlot(row=0, col=2, title="Quadrature spectrum (Phase shift)")

        p_pwb = graph.ci.addPlot(row=1, col=0)
        p_reb = graph.ci.addPlot(row=1, col=1, title="Co-spectrum (In-phase)")
        p_imb = graph.ci.addPlot(row=1, col=2, title="Quadrature spectrum (Phase shift)")
        ...
        for p in all_plots:
            ...
            if p in (p_rea, p_ima, p_reb, p_imb):
                zero_line = pg.InfiniteLine(pos=0, angle=0, pen=pg.mkPen("gray", width=1.5, style=Qt.PenStyle.DashLine))
                p.addItem(zero_line)
        ...
        for pol_name, row_plots in [("Pol A", (p_pwa, p_rea, p_ima)), ("Pol B", (p_pwb, p_reb, p_imb))]:
            ...
            if len(peaks) > 0:
                ...
                for px in peak_x:
                    vl_re = pg.InfiniteLine(pos=px, angle=90, pen=pg.mkPen("r", width=1.5, style=Qt.PenStyle.DotLine))
                    p_re.addItem(vl_re)
                    vl_im = pg.InfiniteLine(pos=px, angle=90, pen=pg.mkPen("r", width=1.5, style=Qt.PenStyle.DotLine))
                    p_im.addItem(vl_im)
            else:
